...
namespace STrans.Service.ModesCentre
{
    ...
    {
        private TimeSpan m_tsFetchWaking;
        ...
        public AdminMC(string strMCServiceHost, TimeSpan tsFetchWaking, string jEventListener)
            : base()
        {
            _dictNotify = new Dictionary<ID_EVENT, EventHandler> ();

            m_strMCServiceHost = strMCServiceHost;

            m_tsFetchWaking = tsFetchWaking;

            m_mcSources_EventHandlers = JsonConvert.DeserializeObject<JObject> (jEventListener);
        ...
        private void activateTimerFetchWaking (bool bActivated)
        {
            ...
            _dueTimerFetchWaking = bActivated == true ? (int)m_tsFetchWaking.TotalMilliseconds : System.Threading.Timeout.Infinite;
        ...
        private bool isHandlerMCEvent(ID_EVENT idMCEvent)
        {
        ...
        /// Словарь событий 
        /// </summary>
        private static Dictionary<ID_EVENT, EventHandler> _dictNotify;
        ...
        public void AddEventHandler (ID_EVENT id_event, EventHandler handler)
        {
        ...
        {
            IEventArgs arg = null;
            ID_EVENT id = ID_EVENT.Unknown;
            string mesLog = string.Empty;

            //Func<IEventArgs, ID_EVENT> contextId = delegate (IEventArgs ev) {
            //    ID_EVENT id_event = ID_EVENT.Unknown;

            //    if (typeof (Guid).IsAssignableFrom (ev.) == true)
            //        id_event = ID_EVENT.RELOAD_PLAN_VALUES;
            //    else if (typeof (FormChangeMode.KeyDevice).IsAssignableFrom (ev.m_type) == true)
            //        id_event = ID_EVENT.NEW_PLAN_VALUES;
            //    else
            //        ;
                ...
                case System.Collections.Specialized.NotifyCollectionChangedAction.Remove:
                    ...
                    //??? в случае 'Reset' - исключение, т.к. 'e.OldItems' = NullReference
                        try {
                            ...
                            //Logging.Logg ().Exception (excp, $"AdminMC::listMCEventArgs_CollectionChanged (Act.={e.Action}) - Connected={isConnected}, "
                            //    , Logging.INDEX_MESSAGE.NOT_SET);
                        } finally {
                            activateTimerFetchWaking (true);
                        }
        ...
        public void FetchEvent (bool bRemove)
        {
            try {
                if ((Equals(_listMCEventArgs, null) == false)
                    ...
                    //Logging.Logg ().Warning ($@"trans_mc.AdminMC::FetchEvent (REMOVE={bRemove}) - исходный размер = 0...{Environment.NewLine}Стэк={Environment.StackTrace}"
                    //    , Logging.INDEX_MESSAGE.NOT_SET)
                        ;
            } catch (Exception e) {
                //Logging.Logg ().Exception (e, $@"trans_mc.AdminMC::FetchEvent () - ..."
                //    , Logging.INDEX_MESSAGE.NOT_SET);
            }
        }
                ...
                if (listKey.Count - listKey.Distinct ().Count () == 0) {
                    ...
                    listKey.ForEach ((key) => {
                        if (_listTECComponentKey.Contains (key) == false)
                            ...
                            //Logging.Logg ().Error (string.Format ("trans_mc.AdminMC::PrepareExportRDGValues () - добавление повторяющегося индекса {0}...", key.ToString ())
                            //    , Logging.INDEX_MESSAGE.NOT_SET)
                                ;
                    });
        ...
        {
            string query = Operation.PPBR.ToString();
            int i = -1;
            ...
            //Logging.Logg ().Debug ($"AdminMC::getPPBRValuesRequest (TEC={t.name_shr}, IDevice={comp.name_shr}, Stamp=[{date.ToString()}]) - вЫход...: query=" + query
        ...
        {
            string query = Operation.MaketEquipment.ToString();
            int i = -1;
            ...
            //Logging.Logg ().Debug ($"AdminMC::getMaketEquipmentRequest (Identifiers.Count={listIdentifiers.Count()}, Stamp=[{date.ToString()}]) - вЫход...: query=" + query
            //    , Logging.INDEX_MESSAGE.D_002);
        }
        ...
        protected override int getPPBRValuesResponse(DataTable table, DateTime date)
        {
            ...
            for (i = 0; i < table.Rows.Count; i++)
            {
                try
                {
                    ...
                    //Logging.Logg().Exception(e, string.Format(@"AdminMC::getPPBRValuesResponse () - строка={0}", i)
                    //    , Logging.INDEX_MESSAGE.NOT_SET);
                }
            }
        ...
        private int getMaketEquipmentResponse (DataTable table, DateTime date)
        {
            ...
            if (listKeyDevice.Count > 0) {
                AddEvent (new EventArgs<FormChangeMode.KeyDevice> (ID_EVENT.NEW_PLAN_VALUES, date, listKeyDevice));
            } else
                //Logging.Logg().Warning($@"AdminMC::getMaketEquipmentResponse () - получен пустой список с оборудованием..."
                //    , Logging.INDEX_MESSAGE.NOT_SET)
                    ;
        ...
        private JObject m_mcSources_EventHandlers;
        ...
        private void dbMCSources_OnEventHandler(object obj)
        {
            ...
            Tuple<EVENT, bool> tupleConnectHandler;
            ...
            Func<ID_EVENT, DateTime, DateTime> translate = delegate (ID_EVENT id, DateTime datetime) {
                return ((id == ID_EVENT.RELOAD_PLAN_VALUES) || (id == ID_EVENT.NEW_PLAN_VALUES))
                    ? datetime.SystemToLocalHqEx ()
                        : ((id == ID_EVENT.PHANTOM_RELOAD_PLAN_VALUES) || (id == ID_EVENT.REQUEST_PLAN_VALUES))
                            ? datetime
                                : datetime;
            };

            if (obj is Array) {
                id_event = (ID_EVENT)(obj as object []) [0];

                if (id_event == ID_EVENT.GENOBJECT_MODIFIED) {
                ...
                } else if ((id_event == ID_EVENT.RELOAD_PLAN_VALUES)
                    || (id_event == ID_EVENT.PHANTOM_RELOAD_PLAN_VALUES)) {
                    ...
                    if (isRequired (dateTarget) == true)
                        argEventChanged = new StatisticTrans.Contract.EventArgs<Guid> (id_event, dateTarget, makets);
                    else
                        ...
                        //   , Logging.INDEX_MESSAGE.NOT_SET)
                           ;
                } else if ((id_event == ID_EVENT.NEW_PLAN_VALUES)
                    || (id_event == ID_EVENT.REQUEST_PLAN_VALUES)) {
                    ...
                    //            , id_event == ID_EVENT.NEW_PLAN_VALUES ? "новый" : id_event == ID_EVENT.REQUEST_PLAN_VALUES ? "<запрос>" : "НЕ ИЗВЕСТНО")
                    //    , Logging.INDEX_MESSAGE.NOT_SET);

                    // проверить дату за которую получен новый план: только сегодняшние и следующие сутки сутки
                    if (isRequired (day) == true) {
                        tec = m_list_tec.ToList().Find (t => {
                            return t.name_MC.Trim ().Equals (id_mc_tec.ToString ());
                        });

                        argEventChanged = new StatisticTrans.Contract.EventArgs<FormChangeMode.KeyDevice> (id_event, day, new ReadOnlyCollection<FormChangeMode.KeyDevice> (
                            allTECComponents.ToList().FindAll (comp => {
                ...
                if ((Equals (argEventChanged, null) == false)
                    ...
                    ;
            } else if (typeof (Tuple<EVENT, bool>).IsAssignableFrom (obj.GetType ()) == true) {
                //TODO: ретранслировать для формы произошла подписка/отписка от события Модес-Центра
                tupleConnectHandler = obj as Tuple<EVENT, bool>;

                _dictNotify [ID_EVENT.HANDLER_CONNECT]?.Invoke (this, new EventArgs<bool>(EventArgs.Empty.TranslateEvent (tupleConnectHandler.Item1), DateTime.MinValue, new ReadOnlyCollection<bool> (new List<bool>() { tupleConnectHandler.Item2 })));
            } else if (typeof (bool).IsAssignableFrom (obj.GetType ()) == true) {
                id_event = ID_EVENT.Unknown;

                //TODO: проверить результат попытки установки соединения
                //Logging.Logg ().Action ($"AdminMC::dbMCSources_OnEventHandler(ID_MC_EVENT={id_event.ToString ()}) - изменение состояния соединения УСТАНОВЛЕНО = {(bool)obj})..."
                //    , Logging.INDEX_MESSAGE.NOT_SET);
        ...
        public void DebugEventReloadPlanValues ()
        {
            Action doWork = delegate () {
                ...
                identifiers.ForEach (guid => {
                    ...
                    dbMCSources_OnEventHandler (new object [] { ID_EVENT.PHANTOM_RELOAD_PLAN_VALUES
        ...
        public void ToDateRequest (DateTime date, int ms_sleep = 567)
        {
            Action doWork = delegate () {
                ...
                foreach (TEC t in m_list_tec) {
                    ...
                    dbMCSources_OnEventHandler (new object [] { ID_EVENT.REQUEST_PLAN_VALUES
