...
namespace StatisticCommon
{
    public abstract class HAdmin : HStates
    {
            ...
            if (!(m_list_tec == null))
                foreach (TEC t in m_list_tec)
                    if (!(t.connSetts == null))
                    {
                        ...
                        if (((t.m_markQueries.IsMarked ((int)CONN_SETT_TYPE.DATA_AISKUE) == true) || (t.m_markQueries.IsMarked ((int)CONN_SETT_TYPE.DATA_SOTIASSO_INSTANT) == true) || (t.m_markQueries.IsMarked ((int)CONN_SETT_TYPE.MTERM) == true)) &&
        ...
        protected string getNamePBRNumber (int hour = -1) {
            return @"ПБР" + this.GetPBRNumber (hour);
        }
