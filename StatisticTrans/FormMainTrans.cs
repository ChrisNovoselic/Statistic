    ...
    public abstract partial class FormMainTrans : FormMainStatistic
    {
        ...
        protected virtual void setDataGridViewAdmin(DateTime date, bool bNewValues, IList<HAdmin.RDGStruct> values)
        {
                ...
                && (m_bEnabledUIControl == false))
            {
                try
                {
                    ...
                    //Logging.Logg().Exception(e, string.Format(@"FormMainTrans::setDataGridView (CurrentKey={0}) - ...", _client.GetCurrentKey(m_IndexDB).ToString()), Logging.INDEX_MESSAGE.NOT_SET);
                }
        ...
        protected virtual void SaveRDGValues (bool bCallback)
        {
            new Thread (new ParameterizedThreadStart (delegate (object obj) {
                try {
                    _client.SaveRDGValues (CONN_SETT_TYPE.DEST, (PARAMToSaveRDGValues)obj);
                } catch (Exception e) {
                    Logging.Logg ().Exception (e, @"FormMainTrans::saveRDGValues () - ", Logging.INDEX_MESSAGE.NOT_SET);
                }
            })).Start (new PARAMToSaveRDGValues (SelectedItemKey, dateTimePickerMain.Value, bCallback));
        }
        ...
        protected virtual void saveDataGridViewAdminComplete(int state)
        {
            ...
            if ((IsService == true)
                && (m_bEnabledUIControl == false))
            {
                _client.TECComponentComplete (CONN_SETT_TYPE.SOURCE, state, true);
        ...
        private void _callback_EventRaise (ServiceCallbackResultEventArgs arg1)
        {
            DateTime date;
            bool bResult = false;
            ...
            Logging.Logg ().Debug ("::_callback_EventRaise () - ...", Logging.INDEX_MESSAGE.NOT_SET);
        }
