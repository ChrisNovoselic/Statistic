...
namespace StatisticCommon
{
    public class AdminTS_NSS : AdminTS
    {
        ...
        public override void GetRDGValues(int id)
        {
            //delegateStartWait ();
        ...
        private void threadGetRDGValuesWithDate(object date)
        {
            ...
            //{
                foreach (int indx in m_listTECComponentIndexDetail)
                {
                    indxEv = WaitHandle.WaitAny(m_waitHandleState);
                    if (indxEv == 0)
                        base.GetRDGValues(indx, (DateTime)date);
                    else
                        break;
                }
            //}
        ...
        public override void GetRDGValues(int id, DateTime date)
        {
            //delegateStartWait ();
            fillListIndexTECComponent (id);
