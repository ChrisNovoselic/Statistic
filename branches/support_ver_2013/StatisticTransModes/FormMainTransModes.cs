...
namespace StatisticTransModes
{
    public abstract class FormMainTransModes : FormMainTrans
    {
        public FormMainTransModes(int id_app)
            : base(id_app
                    , new string[] { @"ИгнорДатаВремя-techsite" }
                ...
                case FormChangeMode.MODE_TECCOMPONENT.GTP:
                    switch (m_IndexDB)
                    {
                        case (Int16)CONN_SETT_TYPE.SOURCE:
                            m_arAdmin[m_IndexDB].GetRDGValues(m_listTECComponentIndex[comboBoxTECComponent.SelectedIndex], dateTimePickerMain.Value.Date);
                            break;
