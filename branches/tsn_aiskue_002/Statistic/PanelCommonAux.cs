...
namespace Statistic
{
    ...
    //    }
        ...
        public ConnectionSettings GetConnSettAIISKUECentre()
        {
            ...
            string[] connSettPars = GetSecValueOfKey(SEC_CONFIG, @"AIISKUE_CONNECT").Split(s_chSecDelimeters[(int)INDEX_DELIMETER.PAIR_VAL])
                , pars;

            foreach (string pairPars in connSettPars)
            {
                pars = pairPars.Split(s_chSecDelimeters[(int)INDEX_DELIMETER.VALUE]);

                switch (pars[0])
                {
                    case @"ID":
                        connSettRes.id = Int32.Parse(pars[1]);
                        break;
                    case @"NAME":
                        connSettRes.name = pars[1];
                        break;
                    case @"SERVER":
                        connSettRes.server = pars[1];
                        break;
                    case @"PORT":
                        connSettRes.port = Int32.Parse(pars[1]);
                        break;
                    case @"DB_NAME":
                        connSettRes.dbName = pars[1];
                        break;
                    case @"UID":
                        connSettRes.userName = pars[1];
                        break;
                    case @"PSWD*":
                        connSettRes.password = pars[1];
                        break;
                    default:
                        break;
                }
    ...
    partial class PanelCommonAux : PanelStatistic
    {
        ...
        protected string setListTEC()
        {
            ...
            m_listTEC = m_fileINI.GetListTEC();
            ...
            //TEC.s_strQueryTemplate[(int)TEC.INDEX_DATA.TG] = m_fileINI.GetQueryTemplateSignals(Common.TEC.INDEX_DATA.TG);
            //TEC.s_strQueryTemplate[(int)TEC.INDEX_DATA.TSN] = m_fileINI.GetQueryTemplateSignals(Common.TEC.INDEX_DATA.TSN);
            //TEC.s_strQueryTemplate[(int)TEC.INDEX_DATA.GRII] = m_fileINI.GetQueryTemplateSignals(Common.TEC.INDEX_DATA.GRII);
            //TEC.s_strQueryTemplate[(int)TEC.INDEX_DATA.GRVI] = m_fileINI.GetQueryTemplateSignals(Common.TEC.INDEX_DATA.GRVI);
            //TEC.s_strQueryTemplate[(int)TEC.INDEX_DATA.GRVII] = m_fileINI.GetQueryTemplateSignals(Common.TEC.INDEX_DATA.GRVII);
            TEC_LOCAL.s_strQueryTemplate = m_fileINI.GetQueryTemplateSignals();
