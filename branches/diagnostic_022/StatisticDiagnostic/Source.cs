    ...
    partial class PanelStatisticDiagnostic
    {
        private class PanelContainerTec : HPanelCommon
        {
            ...
            partial class PanelTec
            {
                ...
                public void Clear()
                {
                    for (int j = 0; j < m_dgvTesting.Rows.Count; j++)
                        if (m_dgvTesting.Rows.Count > 0)
                            m_dgvTesting.Rows.Clear();
                        else
                            ;
                }
                ...
                private void toolStripMenuItemActivate_Click(object sender, EventArgs e)
                {
                    ...
                    for (int i = 0; i < m_dgvTesting.Rows.Count; i++)
                        if (m_dgvTesting.Rows[i].Cells[(int)INDEX_CELL.NAME_SOURCE].Style.BackColor == s_ColorSOTIASSOState[(int)INDEX_CONTEXTMENU_ITEM.ACTIVATED])
                            paintCellDeactive(i);
                        else
                            ;
                ...
                private void addRows(int cntRow)
                {
                    Action<int> actionAddRows = new Action<int>((cnt) => { m_dgvTesting.Rows.Add(cnt); });

                    if (m_dgvTesting.RowCount < cntRow / 2)
                        if (m_dgvTesting.InvokeRequired)
                            m_dgvTesting.Invoke(actionAddRows, cntRow / 2);
                        else
                ...
                private void fillValues(string filter)
                {
                    ...
                    for (int r = 0, t = 0; r < m_dgvTesting.Rows.Count; r++, t += 2)
                    {
                        nameSource = m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.NAME_SOURCE].Value.ToString();

                        shortTime = formatTime(drTecSource[t + 1]["Value"].ToString(), nameSource);
                        m_dgvTesting.Invoke(new Action(() => m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.DATETIME_VALUE].Value = shortTime));
                        m_dgvTesting.Invoke(new Action(() => m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.VALUE].Value = drTecSource[t]["Value"]));
                        m_dgvTesting.Invoke(new Action(() => m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.DATETIME_NOW].Value =
                        ...
                        m_dgvTesting.Invoke(new Action(() => m_dgvTesting.Rows[r].Tag = drTecSource[t]["NAME_SHR"]));
                ...
                public void AddItem()
                {
                    ...
                    for (int i = 0; i < m_tableTECList.Rows.Count; i++)
                    {
                        ...
                        addRows(m_tableSourceData.Select(filter).Length);
                ...
                private void setTextColumn()
                {
                    ...
                    Action<int, string> setTextNameSource =
                        new Action<int, string>((indx, name_shr) => { m_dgvTesting.Rows[indx].Cells[(int)INDEX_CELL.NAME_SOURCE].Value = name_shr; });
                    ...
                    //{
                        ...
                        for (int j = 0; j < m_dgvTesting.Rows.Count; j++)
                        {
                            ...
                            if (m_dgvTesting.InvokeRequired == true)
                                m_dgvTesting.Invoke(setTextNameSource, j, arSelParamDiagnostic[0]["NAME_SHR"].ToString());
                            else
                ...
                private void cellsPingTEC()
                {
                    ...
                    Action<int, INDEX_STATE> setState = new Action<int, INDEX_STATE>((iRow, iState) => {
                        m_dgvTesting.Rows[iRow].Cells[(int)INDEX_CELL.STATE].Value = s_StateSources[(int)iState].m_Text;
                        m_dgvTesting.Rows[iRow].Cells[(int)INDEX_CELL.STATE].Style.BackColor = s_StateSources[(int)iState].m_Color;
                    });

                    arSel = m_tableSourceData.Select(@"ID_EXT = " + Convert.ToInt32(m_tec.m_id));

                    for (int i = 0, t = 0; i < m_dgvTesting.Rows.Count; i++, t += 2)
                        if (m_dgvTesting.InvokeRequired == true)
                            m_dgvTesting.Invoke(setState, i, (arSel[t]["Link"].ToString().Equals("1") == true) ? INDEX_STATE.OK : INDEX_STATE.ERROR);
                        else
                ...
                {
                    if (m_dgvTesting.InvokeRequired)
                        m_dgvTesting.Invoke(new Action(() => m_dgvTesting.Rows[y].Cells[(int)INDEX_CELL.NAME_SOURCE].Style.BackColor = clrCell));
                    else
                        m_dgvTesting.Rows[y].Cells[(int)INDEX_CELL.NAME_SOURCE].Style.BackColor = clrCell;
                }
                ...
                {
                    string a = m_dgvTesting.Rows[y].Tag.ToString();
                    string b;
                ...
                {
                    string nameSource = m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.NAME_SOURCE].Value.ToString();

                    if ((!(nameSource == "АИИСКУЭ"))
                        && m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.NAME_SOURCE].Style.BackColor == System.Drawing.Color.Empty)
                ...
                {
                    m_dgvTesting.Rows[r].Cells[(int)INDEX_CELL.VALUE].Style.BackColor = bflag == true ? Color.Firebrick : Color.Empty;
                }
